...
router.get('/stats', asyncHandler(async (req, res) => {
  ...
  const mrr = await prisma.user.aggregate({
    ...
    _sum: {
      // This would need a price field; using calculation instead
    }
  });
...
router.post('/properties/:id/comments', asyncHandler(async (req, res) => {
  const { id } = req.params;
...
// Scraper jobs
router.get('/scraper-jobs', asyncHandler(async (req, res) => {
...
router.post('/scraper-jobs', asyncHandler(async (req, res) => {
  const schema = z.object({
